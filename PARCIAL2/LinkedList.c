 ...
 */
void ll_startIterator(LinkedList* this)//int 0
{
    if(this != NULL)
    {
...
/** \brief Cuenta la cantidad de algo evaluando por una funcion filto
 * \param pList LinkedList* Puntero a la lista
 * \param pFunc (*pFunc) Puntero a la funcion criterio que devuelve un 1 si paso el filtro
 * \param char* valueFiltro es el valor que se la pasa a la funcion criterio
 * \return int Retorna la cantidad encontrada de elementos que pasan el filtro
 */
int ll_count(LinkedList* this,int (*pFunc)(void* pElement, char* filtro), char* valueFiltro)
{
    void* auxElement = NULL;
    int returnAux = -1;
    int i;
    if(this != NULL && ll_len(this) > 0 && pFunc != NULL && valueFiltro != NULL)
    {
        returnAux = 0;
        ll_startIterator(this);
        for(i=0; i<ll_len(this); i++)
        {
            auxElement = ll_getNext(this);
            returnAux += (*pFunc)(auxElement, valueFiltro);
        }
    }
    return returnAux;
}
/** \brief Cuenta la cantidad de algo evaluando por una funcion que devuelve un numero
 * \param pList LinkedList* Puntero a la lista
 * \param pFunc (*pFunc) Puntero a la funcion que nos retorna el numero que se suma al acumulador
 * \return int Retorna la cantidad acumulada
 */
int ll_count_(LinkedList* this,int (*fn)(void* element))
{
    void* auxElement = NULL;
    int returnAux = -1;
    int i;
    if(this != NULL && ll_len(this) > 0 && fn != NULL)
    {
        returnAux = 0;
        ll_startIterator(this);
        for(i=0; i<ll_len(this); i++)
